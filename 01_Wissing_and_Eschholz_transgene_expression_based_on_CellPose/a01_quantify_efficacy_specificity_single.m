...
for fileIdx = 1:length(files2analyze)
    ...
    im_green_norm = rescale(double(im_green), 0, 1);                                    % normalize brightness of green channel to maximum
    im_red_norm = rescale(double(im_red), 0, 1);                                        % normalize brightness of red channel to maximum
    ...
    [AvBrightRed_THbased, AvBrightGreen_THbased, co_express_THbased] = f03_coexpress(cp_red, mastermask_green, overlap, im_red_norm, im_green_norm);      % calculate co-expression based on red channel 
    [AvBrightGreen_GFPbased, AvBrightRed_GFPbased, co_express_GFPbased] = f03_coexpress(cp_green, mastermask_red, overlap, im_green_norm, im_red_norm);   % calculate co-expression based on green channel 
